...
export function authHeaders() {
  const headers = {
    'User-Agent': 'flywheel-security-scan',
...
export async function scanRepo(identifier) {
  ...
  const dependabot = !!(await fetchJSON(
    `https://api.github.com/repos/${owner}/${name}/contents/.github/dependabot.yml${refParam}`
  ));
...
export function updateMarkdown(existing, table) {
  ...
  const start = lines.findIndex((l) =>
    l.startsWith('## Security & Dependency Health')
  );
  if (start !== -1) {
    let end = start + 1;
  ...
  const coverage = lines.findIndex((l) =>
    l.startsWith('## Coverage & Installer')
  );
  if (coverage !== -1) {
...
export async function main(options = {}) {
  ...
  const repoList = (await readFile(listPath, 'utf8'))
    .trim()
    .split('\n')
    .filter(Boolean);
  const results = [];
