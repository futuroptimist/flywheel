...
- The `@codex` comment must be actionable:
  - Start with a Scope Lock recap: “Only touch X/Y/Z; do not touch A/B/C; keep diff small; run these commands.”
...
- Prefer PRs that touch fewer files and are localized to the bug surface; penalize PRs that add new features, refactor unrelated modules, or add non-required tests.
...
- Prefer regression tests that assert the user-visible contract directly; avoid brittle heuristics (e.g., “widest element” scans, visualViewport math hacks) unless the prompt explicitly asks for them. If tests are flaky, fix the layout cause rather than loosening tests.
...
- If the input comment includes scope constraints, keep them explicit as “Allowed changes” / “Do not touch”.
