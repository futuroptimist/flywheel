...
- Require each applicable Codecov check to complete successfully before returning the unconditional category 1 response. A failed, errored, cancelled, timed-out, stale, missing, or action-required Codecov result is a merge blocker and must not be waived under the demonstrably unrelated CI exception.
...
- Preserve strict Codecov handling: inspect every latest-head Codecov check or status (including patch, project, flag, and component checks), require every applicable Codecov result to pass even if it is not marked required or another coverage result passes, never apply the unrelated-CI exception to it, and use the target reported by Codecov or configured by that repository rather than assuming a fixed percentage.
