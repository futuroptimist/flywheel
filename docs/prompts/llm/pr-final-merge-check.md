...
- Highest priority: if code, tests, configuration, generated artifacts, documentation in the repository, or any other repository changes are still needed for merge readiness, return category 2. If the PR description also needs work, assess and track the known description problem in category 2's tally, but do not ask Codex to change PR metadata or generate the replacement description until repository changes are complete and this merge check is rerun.
...
- If the PR is not ready to merge because repository changes are still needed, respond with exactly two elements in this order and no other prose before, between, or after them:
...
- Maintain a persistent merge-readiness tally across repeated invocations in the same LLM conversation:
  ...
  - Retain completed and unresolved items so the history remains in context, preserving item wording and ordering when practical.
...
- Target one or a small, coherent, reliably executable group of currently unchecked repository-work items. Group tightly related items when safe to reduce unnecessary commits, but do not overload one Codex task with unrelated work.
...
- Include a "Reviewer comment resolution" section that maps each substantive concern selected for the current bounded batch to the concrete repository change or durable in-code justification needed.
- For each selected concern, ask Codex to either implement the reviewer’s suggested change or add enough durable justification in the repository to address the concern and proceed without further changes. The tally remains the authoritative record of all known blockers, including blockers not selected for the current task.
...
- Keep the comment fully self-contained without mentioning the tally, checkbox states, item numbers, or phrases such as "the item above."
