...
- Required checks, CI status, pending checks, failed checks, skipped checks that look relevant, and stale results, including every Codecov check and status on the latest head.
...
- Treat every Codecov check or status on the latest head as a relevant CI check, including `codecov/patch`, `codecov/project`, and any flag or component checks. Do this even when GitHub does not label the check as required, another coverage check passes, or the PR's overall checks summary appears green.
- Treat a Codecov result as applicable when it exists or the effective repository workflow and configuration indicate that it should be emitted for the latest head. Treat a nonexistent result as non-applicable only when the workflow or configuration and the PR's event and security context independently establish that its upload was intentionally skipped or unavailable, such as a tokenless fork; an author assertion or unexplained absence is insufficient.
- Require each applicable Codecov check to complete successfully before returning the unconditional category 1 response. A failed, errored, cancelled, timed-out, stale, unexpectedly missing, or action-required Codecov result is a merge blocker and must not be waived under the demonstrably unrelated CI exception. An independently verified, intentionally non-emitted upload is not a missing applicable result and does not require a category 2 repository-change task.
- For a queued or in-progress Codecov check, use the conditional category 1 response only when it satisfies the same narrow pending-CI rule as every other check.
...
- Preserve strict Codecov handling: inspect every latest-head Codecov check or status (including patch, project, flag, and component checks), require every existing or expected Codecov result to pass even if it is not marked required or another coverage result passes, never apply the unrelated-CI exception to it, and use the target reported by Codecov or configured by that repository rather than assuming a fixed percentage. Treat a nonexistent result as non-applicable only when the effective workflow or configuration and the PR's event and security context independently prove its upload was intentionally skipped or unavailable; do not accept an author assertion or unexplained absence, and keep unexpectedly missing expected results blocking without forcing repository work for independently verified, intentionally non-emitted uploads.
