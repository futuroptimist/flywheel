...
Apply the scaffolding for supported suggestions with `--apply`. Use
`--apply-all` to apply supported changes without any prompts—it's equivalent to
passing `--apply --yes` and creates a starter `README.md`, a `docs/README.md`
placeholder, and a `tests/.gitkeep` marker when the corresponding heuristics are
triggered. Suggestions that still require manual intervention remain listed in
the skipped summary so you can address them after the automated scaffolding
completes.
